...
최신 v3를 고정 주소에 반영했고, 승인된 원본 300장을 포함한 **업무 사진 357장**을 조회할 수 있습니다. 계획·업로드·자동 보수·ROI 화면을 실제 브라우저와 백엔드 파일로 검증했습니다. 실행 근거는 [최종 인수 기록](records/2026-09-22-product-v3-status.md)에 남겼습니다.
...
| ROI·SHAP | 영역별 판독, 실제 모델의 양·음 기여도와 대상 등급 | 실제 SHAP·영역 표시·입력 잠금·늦은 응답·복원 독립 검증 PASS |
...
| 원본 300장 | 사진 기록 300개·고유 객체 299개·483,481,091바이트 전수 SHA 대조 | v3 후보 복제·4개 서비스 재시작 후 보존, 최종 공개 사이트 300개 ID 포함 확인 |
| ROI·실제 SHAP | 실제 기여도 50,176개·잔차 재계산, 선택·표시 강도·저장 잠금·늦은 응답·숨김/복원 | v3 모델 API와 후보 B 독립 UI 검증 PASS |
| v3 핵심 업무 | 포인트 없는 계획·추가 업로드·자동 보수·수정·복원·충돌·이력 | 후보 A 실제 UI/API/파일 검증 PASS |
| 고정 주소 | 청크 업로드·같은 ID 재시도·실제 AI·원본 SHA·이력, 연결 재시작 뒤 같은 URL | 고정 경로 실전송·재연결 PASS, 최신 v3에서 기존 파일·판독·이력 재조회 PASS |
...
- 사용자가 승인한 원본 300장과 기존 업무 기록을 최종 사이트에 연결했습니다. 검증·시연 목적으로 분류한 기록은 일반 업무 목록에서 분리하고 기록은 보존합니다.
...
| [핵심 업무 검증](records/validation-v3-core-a-2026-09-22.md) · [ROI 회귀 검증](records/validation-v3-roi-b-2026-09-22.md) | 새 화면·API·원본 파일을 대조한 결과 |
...
제출 통합본에서 `npm run typecheck`, `npm run build`가 통과했고 `npm test`는 71 PASS / 10 명시적 SKIP입니다. 별도 실행이 필요한 HTTP·격리 통합 검사는 기본 명령에서 건너뛰며, 실제 서비스·브라우저 검증 결과는 위 기록에 구분합니다.
